...
3. Domian 속성 : 쿠키의 domain 속성에 의해서 지정된 도메인 명은 후방 잋리가 된다.
4. Secure 속성 : 쿠키의 secure 속성은 웹페이지가 HTTPS에서 열렸을 떄에만 쿠키 송출을 제한하기 위해서 지정합니다.
5. HTTPOnly 속성 : 키키의 HttpOnly 속성은 자바스크립트를 경유해서 쿠키를 취득하지 못하도록 하는 쿠키의 확장 기능입니다.
...
+ 보통 HTTP는 직저접 TCP와 통신하지만 SSL을 사용한 경우에는 HTTP는 SSL와 통신하고 SSL이 TCP와 통신하게 됩니다. 즉, SSL이라는 껍질을 덮어쓴
HTTP가 HTTPS인 것입니다.
...
+ 두 개의 키를 사용하는 공개키 암호
  ...
  - 암호를 보내는 측이 상대의 공개키를 사용해 암호화를 합니다. 그리고 암호화된 정보를 받아들인 ㅅ아대는 자신의 비밀키를 사용해 복호하를 실시합니다. 이방식은 암호를 푸는 비밀키를 토인으로 보낼 필요가 없기 때문에 도청에 의해서 키를 빼앗길 걱정은 없습니다.
...
2. 리퀘스트는 클라이언트에서만 시작할 수 있다. 리스폰스만 받는 거슨 불가능하다.
